...
import SalidaIntencion from './SalidaIntencion'
import MuroEnVivo from './MuroEnVivo'
import BarraFija from './BarraFija'
import type { Variante } from '@/data/clase-variantes'
import {
  CLASE,
  CREDENCIALES,
  GUIA,
 ...
 *
 * ── Una página, cinco variantes ─────────────────────────────────────────────
 * Recibe una `Variante` y con ella arma la entrada: el chip, el titular, la
 * bajada y el bloque del problema. **Todo lo demás es idéntico en las cinco**
 * — recortes, reseñas, contador, formulario y preguntas— porque si cambiaran
 * dos cosas a la vez no se sabría cuál movió la aguja y el A/B dejaría de
 * medir algo. Ver `data/clase-variantes.ts`.
 *
 * El slug de la variante viaja hasta el CRM como etiqueta `variante:<slug>`:
 * sin eso, Meta dice quién hizo clic pero nadie sabe quién se registró.
 *
...
export default function PaginaClase({ variante }: { variante: Variante }) {
  return (
    <>
      {/* ───────────────── Barra de arriba ───────────────── */}
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 pb-20 pt-16 text-center md:px-12 md:pt-24">
          ...
          <p className="inline-flex items-center gap-2 rounded-full border border-brand-accent/40 bg-brand-accent/10 px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.16em] text-brand-accent-light">
            ...
            {variante.kicker || `Clase gratuita en vivo · ${CLASE.fechaTexto}`}
          </p>

          <h1 className="mt-7 text-[40px] font-bold leading-[1.05] tracking-tight text-brand-text sm:text-6xl md:text-7xl">
            {variante.h1.linea1}
            <br />
            <span className="text-brand-accent">{variante.h1.linea2}</span>
          </h1>
      ...
      <section id="registro" className="relative overflow-hidden py-20 scroll-mt-20">
        ...
        <div className="relative z-10 mx-auto max-w-lg px-6 md:px-12">
          ...
          <div className="mt-8 rounded-3xl border border-brand-accent/25 bg-brand-card/90 p-6 shadow-[0_0_60px_rgba(139,92,246,0.15)] sm:p-8">
            <FormularioClase origen={variante.slug} />
          </div>
      ...
      <BarraFija />
      <SalidaIntencion origen={variante.slug} />
    </>
  )
}
